...
const platforms = [
  new Platform({ x: -2, y: 585 }),
  new Platform({ x: 628, y: 585 }),
...
function animate() {
  ...
  obstacles.forEach((obstacle) => {
    if (
      ...
      obstacle.position.y < player.position.y + player.height &&
      obstacle.height + obstacle.position.y > player.position.y
    ) {
      location.reload();
    }

    if (
      dish.position.x < player.position.x + player.width &&
      dish.position.x + dish.width > player.position.x &&
      dish.position.y < player.position.y + player.height &&
      dish.height + dish.position.y > player.position.y
    ) {
      dish.position.y = 900;
    }
